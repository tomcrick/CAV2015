...
For many --- like CAV this year --- this takes the form of the author
providing \emph{artefacts} (an accessible tool for reproducing
results) to evaluate. Journals such as Nature, PLoS Computational
Biology and Bioinformatics explicitly require that source code and
data is made available online under some form of open source
license. While these initiative are great, they are often optional,
seem piecemeal, and do little to enable verification or validation of
...
% How best to secure a service which compiles and runs arbitrary code on
% arbitrary inputs?
There are clearly security concerns around providing open
e-infrastructure that pulls, compiles and runs arbitrary
code as an autonomous continuous integration framework...

\subsection{Cloud performance}

Another issue is around performance comparisons of benchmarks from
cloud computations: how can we estimate and compare raw performance in
the cloud? While there has been some work on verifying computations
without reexecuting them~\cite{walfish+blumberg:2015}, projects such
...
explicitly-defined three year schedule for universal adoption.
